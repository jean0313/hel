...
int APIENTRY WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance,
               LPSTR lpCmdLine, int nCmdShow )//启动服务器的入口函数
{
	// Install SEH
	// SetUnhandledExceptionFilter((LPTOP_LEVEL_EXCEPTION_FILTER)lpTopLevelExceptionFilter);
	sprintf( szAppClass, "游戏服务器%d", hInstance);
	if (!InitApplication( hInstance))		return (FALSE);//初始化应用程序
    if (!InitInstance(hInstance, nCmdShow)) return (FALSE);//初始化应用实例
...
{     
 WNDCLASS  wc;
	...
	wc.style = (CS_HREDRAW | CS_VREDRAW | CS_OWNDC | CS_DBLCLKS); //窗口样式
	wc.lpfnWndProc   = (WNDPROC)WndProc;       //窗口回调函数  也就是上次发的 Wmain的核心函数之一          
	wc.cbClsExtra    = 0;                            
	wc.cbWndExtra    = sizeof (int);             
	wc.hInstance     = hInstance;                    
	wc.hIcon         = NULL;                         //光标默认
	wc.hCursor       = LoadCursor(NULL, IDC_ARROW);  //指针为箭头
	wc.hbrBackground = (HBRUSH)(COLOR_WINDOW+1);	 //背景颜色
	wc.lpszMenuName  = NULL;                    	//菜单名字	 
	wc.lpszClassName = szAppClass;                   //创建窗口名字
...
// HANDLE hFile;
 SYSTEMTIME SysTime;
	...
	wsprintf(cTitle, "战场游戏服务器 V%s.%s %d (运行时刻: %d年-%d月-%d日-%d时)", DEF_UPPERVERSION, DEF_LOWERVERSION, DEF_BUILDDATE, SysTime.wYear, SysTime.wMonth, SysTime.wDay, SysTime.wHour);
	//在窗口标题栏上显示 战场游戏服务器 V2.224 (运行时刻: %d年-%d月-%d日-%d时) 汉化就是这样弄的 改成年月日更适合中国人
	G_hWnd = CreateWindowEx(0,  // WS_EX_TOPMOST,//建立窗口
        szAppClass,  //窗口名字
        cTitle,
		...
		CW_USEDEFAULT,
        CW_USEDEFAULT,
        800, //GetSystemMetrics(SM_CXSCREEN),//窗口大小
