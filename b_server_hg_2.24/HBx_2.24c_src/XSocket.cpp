...
BOOL XSocket::bListen(char * pAddr, int iPort, unsigned int uiMsg)//솰桂
{
 SOCKADDR_IN	 saTemp;

	if (m_cType != NULL) return FALSE;
	if (m_Sock  != INVALID_SOCKET) closesocket(m_Sock); //呵겨밑균敬쌈俚
	...
	// 주소를 바인드한다.
	memset(&saTemp,0,sizeof(saTemp));
	saTemp.sin_family = AF_INET; //극伎槨AF_INET
	saTemp.sin_addr.s_addr = inet_addr(pAddr); //IP
	saTemp.sin_port = htons(iPort);//똥왯
	if ( bind(m_Sock, (PSOCKADDR)&saTemp, sizeof(saTemp)) == SOCKET_ERROR) {//곬땍敬쌈俚
		closesocket(m_Sock);
		return FALSE;
	}
	
	if (listen(m_Sock, 5) == SOCKET_ERROR) {//솰桂
		closesocket(m_Sock);
		return FALSE;
	}
...
BOOL _InitWinsock()
{
 ...
 WSADATA wsaData;
	...
	wVersionRequested = MAKEWORD( 2, 2 ); //속潼敬쌈俚경굶 2 2 槨君瞳離멕돨경굶
