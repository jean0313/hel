...
BOOL CMap::bSetItem(short sX, short sY, class CItem * pItem)
{
 ...
 register int i;
	...
	// v1.4 付瘤阜 鸥老俊 酒捞袍捞 乐促搁 昏力矫虐绊 捞悼茄促. 
...
BOOL CMap::bInit(char * pName)
{
 int i;
	...
	if (_bDecodeMapDataFileContents() == FALSE) //读取地图信息
		return FALSE;	
...
BOOL CMap::_bDecodeMapDataFileContents()
{
 ...
 short * sp;
	...
	ReadFile(hFile, (char *)cHeader, 256, &nRead, NULL);//从文件一次读取256字节,存放在cHeader中



	// 刚历 澄巩磊甫 绝矩促.
	for (i = 0; i < 256; i++) //如果空 就把他赋值成" ".
		if (cHeader[i] == NULL) cHeader[i] = ' ';

	cReadMode = 0;

	pStrTok = new class CStrTok(cHeader, seps); //new 一个CStrTok类的对象，来调用pGet()获取文件信息
	token = pStrTok->pGet();
	...
	while( token != NULL )   {
		...
		if (cReadMode != 0) {
			switch (cReadMode) {
			case 1:
				m_sSizeX = atoi(token); //获取m_sSizeX大小
				cReadMode = 0;
				break;
			case 2:
				m_sSizeY = atoi(token); //获取m_sSizeY大小
				cReadMode = 0;
				break;
			case 3:
				m_sTileDataSize = atoi(token);//获取m_sTileDataSize大小
				cReadMode = 0;
				break;
			}
		}	
		else {//和上次说的那个文件获取信息代码类似。
	...
	m_pTile = (class CTile *)new class CTile[m_sSizeX * m_sSizeY]; //new 一个CTile类对象 下面他要做文章了
    //大家可以看看CTile这个类只有数据成员，没有函数。类似结构体功能。
...
int CMap::iAnalyze(char cType, int * pX, int * pY, int * pV1, int * pV2, int * pV3)
{
	...
	// 泅犁 甘狼 惑炔阑 盒籍窍咯 孽府俊 嘎绰 困摹甫 馆券茄促. 
	switch (cType) {
	case 1:
		// 泅犁 背傈捞 国绢瘤绊 乐绰 镑狼 困摹甫 茫绰促. 
	...
	for (i = 0; i < DEF_MAXNMR; i++) {
		if ((m_rcNoAttackRect[i].top > 0)) {
			...
			for (iy = m_rcNoAttackRect[i].top; iy <= m_rcNoAttackRect[i].bottom; iy++) {
				pTile = (class CTile *)(m_pTile + ix + iy*m_sSizeY);
				pTile->m_iAttribute = pTile->m_iAttribute | 0x00000004;		// 3锅掳 厚飘甫 技泼茄促. 救傈瘤措 
			}
		}
